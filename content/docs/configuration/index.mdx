...
| `purge-record-on-start` | `false` | When the engine starts, drop completed/stopped task records from the session. On-disk files for stopped torrent tasks are preserved (the orphan-purge step is told to keep files for those info-hashes); HTTP/FTP/etc. payloads on disk are never touched |
