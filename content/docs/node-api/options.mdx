...
Other options are stored on the task and used the next time a relevant action runs (for example, `dir`/`out` when the task is restarted). To change URIs, the save path, or trackers on an existing task, use [`updateTask`](/docs/node-api/tasks#updatetask) instead.
...
| `split` | `16` | Default number of HTTP connections for new downloads and M3U8 segment workers |
...
| `pbh-enable` | `false` | Start the PeerBanHelper Aria2Next RPC listener (read at `startEngine` only) |
| `pbh-listen-port` | `16801` | PeerBanHelper RPC port (read at `startEngine` only) |
...
| `nzb-body-timeout` | `30` | Maximum idle time in seconds between chunks while fetching an NZB URL body |
