...
`@risuko/risuko-js` is a CommonJS package, so ESM code imports the default export and destructures the functions from it — named imports fail with a `SyntaxError` in plain Node.
...
const { startEngine, addUri, getGlobalStat, onEvent, stopEngine } = risuko;
...
const { startEngine, addUri, tellStatus, onEvent, stopEngine } = risuko;
...
const { startEngine, addUri, changeGlobalOption, onEvent, stopEngine } = risuko;
