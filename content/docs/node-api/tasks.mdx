...
| `kind` | `string` | Protocol family: `"http"`, `"torrent"`, `"ed2k"`, `"m3u8"`, `"ftp"`, `"media"`, ... |
...
BitTorrent tasks include additional fields such as `infoHash`, `bittorrent` (name, comment, announce list), `seeder`, and `numSeeders`. Multi-connection HTTP downloads include `chunkProgress`, an array of `{ completedLength, totalLength }` per chunk.
...
// numWaiting counts waiting and paused downloads; all values are strings
```
