...
- HTTP, media, m3u8, FTP, and similar tasks accept URI, directory, and filename
  changes. Thunder links in `uris` are decoded to HTTP. Changing the primary
  URI, `dir`, or `out` on an active task stops the worker, relocates any
  `.part` file when possible, and starts it again.
- Torrent tasks reject URI, `dir`, and `out` changes. `trackers` appends extra
  announce URLs (comma or newline separated) and is persisted on `bt-tracker`.
- Startup-only option keys (`rpc-*`, `pbh-*`, listen ports, most `bt-*`) are
  rejected on the patch. The desktop **Edit…** dialog uses this API.
...
Each peer object includes aria2-compatible fields (`ip`, `port`, `amChoking`,
