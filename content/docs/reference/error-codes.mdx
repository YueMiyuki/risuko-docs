...
| Code | Name | Description |
|------|------|-------------|
| 540 | yt-dlp Not Found | The bundled `yt-dlp` binary could not be located on `PATH`. Reinstall the app or install `yt-dlp` manually. |
| 541 | Authentication Required | The video requires login. Provide cookies via `load-cookies` or `cookie`. |
| 542 | Format Unavailable | The format selector in `youtube-format` did not match any of the formats yt-dlp returned. |
