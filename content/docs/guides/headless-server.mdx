...
`serve` runs in the foreground until it receives Ctrl+C or an RPC `risuko.shutdown` request. `--rpc-port` defaults to `16800`; pass `-v`/`--verbose` for debug logging. There is no `--rpc-secret` flag — the server reads its secret from config (see [Remote Access](#remote-access)).
