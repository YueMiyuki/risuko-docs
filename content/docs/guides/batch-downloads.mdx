...
- Filenames may contain spaces; path separators (`/`, `\`) are rejected and
  the line is treated as a plain URL when one is present.
...
`thunder://` links and `curl ...` paste-as-task lines are decoded first and
the directive is applied to the resulting URL.
