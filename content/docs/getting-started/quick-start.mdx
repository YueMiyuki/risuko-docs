...
The CLI accepts a magnet link or a path to a `.torrent` file.
...
# With seeding limits (seed-time is in minutes)
risuko download ./ubuntu.torrent --seed-ratio 1.0 --seed-time 60
```

### Download Media

URLs from recognized media hosts are routed through yt-dlp automatically:

```bash
risuko download "https://www.youtube.com/watch?v=..."

# Force yt-dlp for an unrecognized host, with a format selector
risuko download "https://example.com/video" --ytdlp --media-format "bestvideo+bestaudio"
```
