...
| Implementation | Used By | Behavior |
|---------------|---------|----------|
| `NoopEventSink` | Engine `start_engine_headless`, RSS in desktop app, bootstrap upload sinks | Discards events silently |
...
Both `RssManager` and `UploadSinkManager` are constructed from `dyn` trait objects today; `TaskManager` takes a concrete `EventBroadcaster` and a `&Path` for the config directory:
...
let rss = RssManager::new(storage_backend.clone(), event_sink.clone());
let upload_sinks = UploadSinkManager::new(storage_backend, event_sink);
```

Consumers that need to translate broadcast events into a host environment (Tauri, NAPI) subscribe to `events.subscribe()` and forward each `EngineEvent` themselves; the trait surface only matters for `RssManager`, `UploadSinkManager`, and for `start_engine_headless` in `engine/mod.rs`.
