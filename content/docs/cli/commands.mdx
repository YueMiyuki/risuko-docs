...
| `--doh-url` | — | — | DNS-over-HTTPS endpoint URL (e.g. `https://cloudflare-dns.com/dns-query`). Enables DoH for the engine via `changeGlobalOption` before the task is added |
| `--doh-bootstrap` | — | — | Comma-separated IPs used to reach the DoH endpoint without a system-DNS lookup (optional) |
...
# Resolving DNS over HTTPS (applies to the engine globally)
risuko download https://example.com/file.zip \
